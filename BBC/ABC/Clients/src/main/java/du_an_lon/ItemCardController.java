...
public class ItemCardController {
    ...
    public void setData(Item item) throws IOException, URISyntaxException {
        ...
        if(item.getImg() != null && !item.getImg().isEmpty()){
            String urlString = item.getImg();
            Image img = new Image(urlString, true);

            // 3. Đưa vào ImageView (thường là j_img trong JavaFX)
            j_img.setImage(img);
        } else {
            // (Tuỳ chọn) Nếu bạn có 1 cái ảnh mặc định cho sản phẩm chưa có ảnh
            // j_img.setImage(new Image(getClass().getResourceAsStream("/du_an_lon/img/default_product.png")));
        }
