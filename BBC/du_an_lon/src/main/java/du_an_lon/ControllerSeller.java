...
public class ControllerSeller {
    ...
    public void handle_Items() {
        ...
        try {
            ...
            DAOItems.getInstance().Insert(new Electronics(j_name.getText(), j_description.getText(),
            Double.parseDouble(j_StartingPrice.getText()), start, end, p1.getUsername(), j_brand.getText(),j_model.getText(), imageBytes));}
            if(j_ItemType.getValue().equals("Phương tiện giao thông")){
                DAOItems.getInstance().Insert(new Vehicle(j_name.getText(), j_description.getText(),
                        Double.parseDouble(j_StartingPrice.getText()), start, end, p1.getUsername(), j_manufacturer.getText(),j_year.getText(),imageBytes));}
            if(j_ItemType.getValue().equals("Mỹ thuật")){
                DAOItems.getInstance().Insert(new Art(j_name.getText(), j_description.getText(),
                        Double.parseDouble(j_StartingPrice.getText()), start, end, p1.getUsername(), j_artist.getText(),imageBytes));}
    ...
    void handle_SelectImage(ActionEvent event) throws IOException {
        ...
        if (file != null) {
            // Hiển thị ảnh lên ImageView trong Scene Builder
            Image image = new Image(file.toURI().toString());
            imageBytes = Files.readAllBytes(file.toPath());
            MyImgView.setImage(image);

            // Lưu biến file này lại để tí nữa lưu vào Database
        }
    }
